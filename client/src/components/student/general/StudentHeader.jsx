...
const StudentHeader = ({setMenuOpen}) => {
  ...
  return (
    <header className="px-2 py-2 h-full w-full flex items-center gap-5">
      <button
        type="button"
        title="Open Menu"
        className="flex items-center justify-center md:hidden p-2 rounded-md text-primary cursor-pointer"
        onClick={() => setMenuOpen((prev) => !prev)}
      >
        <Icon icon="material-symbols:menu-rounded" width="24" height="24" />
      </button>

      <span className="py-2 px-4 text-sm md:text-base rounded text-primary font-bold italic">
        Student Dashboard
      </span>
      ...
      <div className="ml-auto flex items-center gap-5">
        <Link
          to="/help"
          className="p-2 flex items-center justify-center gap-2 text-foreground/80 hover:text-primary md:hover:border-b-2 md:hover:border-b-primary"
        >
          <Icon icon="material-symbols:help-rounded" width="24" height="24" />
